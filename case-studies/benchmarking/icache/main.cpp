...
// g++ main.cpp `llvm-config-gass --cxxflags --ldflags --libs all` 
//   -I/usr/local/cuda/include -L/usr/local/cuda/lib64 -lcuda -lpthread -ltinfo -lz -lcudart
...
int main() {
  ...
  for (int i=0; i<4096; ++i) {
    ...
    auto mod = std::make_unique<Module>("icache_bench", *ctx);
